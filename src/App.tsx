  ...
  return (
    <header className="sticky top-0 z-50 border-b border-white/10 bg-[#050711]/78 backdrop-blur-2xl">
      <nav className="mx-auto flex min-h-[76px] max-w-7xl items-center justify-between gap-4 px-5 py-4 sm:px-8">
        ...
        <a href="#contacts" className="hidden rounded-full border border-cyan-300/30 bg-cyan-300/10 px-5 py-3 text-sm font-medium text-cyan-50 shadow-glow transition hover:-translate-y-0.5 hover:border-cyan-200/60 hover:bg-cyan-300/20 sm:inline-flex">
          Связаться
        </a>
      </nav>
    </header>
  ...
  return (
    <section className="mx-auto grid min-h-[calc(100vh-76px)] max-w-7xl items-center gap-10 px-5 pb-16 pt-12 sm:px-8 lg:grid-cols-[1.05fr_0.95fr]">
      ...
      <motion.aside
        ...
        className="relative"
      >
        <div className="absolute -inset-8 rounded-[34px] bg-cyan-400/15 blur-3xl animate-pulseGlow" />
        <div className="hero-card animate-float">
          <div className="mb-8 flex items-start justify-between gap-4">
            <div>
              <p className="text-sm text-cyan-200">Portfolio snapshot</p>
              <h2 className="mt-2 text-3xl font-semibold">Максим</h2>
              <p className="mt-2 leading-7 text-slate-300">React, TypeScript, UI, adaptive layout</p>
            </div>
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            {['3 проекта', '13 навыков', 'AI tools'].map((item) => (
              <div key={item} className="rounded-2xl border border-white/10 bg-white/[0.055] px-4 py-4 text-center text-sm text-slate-200">
                {item}
              </div>
            ))}
          </div>
  ...
  return (
    <motion.article variants={fadeUp} whileHover={{ y: -7, scale: 1.01 }} className="glass-panel group">
      <div className="mb-6 flex items-start justify-between gap-4">
        ...
        <div className="hidden rounded-2xl border border-violet-300/25 bg-violet-400/10 p-3 text-violet-100 shadow-violet sm:block">
      ...
      <div className="mb-6 flex flex-wrap gap-2">
        {project.tags.map((tag) => (
          <span key={tag} className="rounded-full border border-white/10 bg-white/[0.055] px-3 py-2 text-xs text-slate-300">
            {tag}
          </span>
        ))}
      </div>
  ...
  return (
    <section id="contacts" className="px-5 py-20 sm:px-8 sm:py-24">
      <motion.div
        ...
        variants={fadeUp}
        className="relative mx-auto max-w-6xl overflow-hidden rounded-[28px] border border-cyan-300/20 bg-white/[0.055] px-6 py-14 text-center shadow-violet backdrop-blur-2xl sm:px-12 sm:py-16"
      >
        <div className="absolute inset-x-8 -top-28 h-56 rounded-full bg-cyan-400/20 blur-3xl" />
        <div className="absolute -bottom-24 right-8 h-52 w-52 rounded-full bg-violet-500/18 blur-3xl" />
    ...
    <section id={id} className="px-5 py-16 sm:px-8 sm:py-20">
      ...
      >
        <motion.div variants={fadeUp} className="mb-10 max-w-3xl">
          <div className="mb-4 inline-flex rounded-full border border-violet-300/25 bg-violet-400/10 px-4 py-2 text-sm text-violet-100">
            {eyebrow}
          </div>
